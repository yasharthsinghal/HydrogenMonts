...
export const meta: MetaFunction = () => {
    return [
        { title: "Checkout & Payment | MONTS" },
        {
            name: "description",
            content:
                "Enter your delivery address and choose Cash on Delivery or Secure Prepaid Payment.",
        },
    ];
};

export async function loader({ context, request }: LoaderFunctionArgs) {
    const { session, cart, customerAccount, storefront, env } =
        await getHydrogenContext(context, request);

    // Helper to fetch authenticated customer profile in parallel with cart
    const fetchCustomerProfile = async () => {
        const customerEmail = session.get("customerEmail") as
            | string
            | undefined;
        if (customerEmail) {
            try {
                return await shopifyCustomerService.getCustomerProfile(
                    storefront,
                    undefined,
                    customerEmail,
                    env,
        ...
        try {
            ...
            if (isLoggedIn) {
                const { data: customerData }: any = await customerAccount.query(
...
export async function action({ request, context }: ActionFunctionArgs) {
    ...
    const paymentMethod = (
        (formData.get("paymentMethod") as string) || "PREPAID"
    ).toUpperCase();
    ...
    const isConsistent = await locationService.validateConsistency(
        zip,
        city,
        province,
    );
    ...
    if (subscribeNewsletter && email) {
        shopifyCustomerService
            .subscribeCustomer(email, "checkout", env)
            .catch((subErr) => {
                console.warn(
                    "[Checkout Subscription Notice] Background sync warning:",
                    subErr,
                );
            });
    }
    ...
    if (paymentMethod === "COD") {
        ...
        const lines =
            ...
            (cartData?.lines?.edges
                ? cartData.lines.edges.map((e: any) => e.node)
                : null) ||
        ...
        if (!codResult.success || !codResult.orderName) {
            return data(
                {
                    error:
                        codResult.errorMessage ||
                        "Unable to place Cash on Delivery order. Please try again or choose online payment.",
                },
                { status: 400 },
            );
        }
        ...
        try {
            ...
            console.warn(
                "[Checkout COD] Notice dispatching confirmation email:",
                emailErr,
            );
        }
        ...
        try {
            ...
            console.warn(
                "[Checkout COD] Notice clearing cart lines:",
                clearErr,
            );
        }
        ...
        return redirect(
            `/thank-you?payment=cod&order=${encodeURIComponent(
                codResult.orderName,
            )}`,
        );
    }
    ...
    return data(
        {
            error: "Unable to initialize online payment gateway. Please try again.",
        },
        { status: 500 },
    );
}
...
export default function CheckoutPage() {
    const { cart, customer } = useLoaderData<typeof loader>() as {
        cart: any;
        customer: any;
    };
    ...
    const [isLocationValid, setIsLocationValid] = useState(() => {
        return Boolean(
            defaultAddr.zip &&
                defaultAddr.city &&
                (defaultAddr.province || defaultAddr.zoneCode),
        );
    });
    ...
    const customerPhone =
        customer?.phoneNumber?.phoneNumber || defaultAddr?.phoneNumber || "";
                                ...
                                <div className='flex flex-col gap-3'>
                                    ...
                                    <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                                        <Input
                                            ...
                                            startIcon={
                                                <Mail className='w-4 h-4 text-[#686764]' />
                                            }
                                        />
                                        ...
                                        <Input
                                            ...
                                            startIcon={
                                                <Phone className='w-4 h-4 text-[#686764]' />
                                            }
                                        />
                                    </div>
                                    ...
                                    <label
                                        ...
                                        <div className='flex flex-col gap-0.5 text-xs select-none'>
                                            <span className='font-semibold text-[#060505] flex items-center gap-1.5 group-hover:text-[#c4622d] transition-colors'>
                                                ...
                                                <span>
                                                    Send me private previews,
                                                    secret catalogs & seasonal
                                                    privilege offers
                                                </span>
                                            </span>
                                            <p className='text-[11px] text-[#686764] leading-relaxed'>
                                                Be the first to access
                                                limited-batch hand-block
                                                releases, early-bird festive
                                                discounts, and digital lookbooks
                                                before public drops. Unsubscribe
                                                anytime in 1 click.
                                            </p>
                                        </div>
                                    </label>
                                ...
                                <div className='flex flex-col gap-3 pt-4 border-t border-[#e8e4df]'>
                                    ...
                                    <div className='grid grid-cols-2 gap-4'>
                                        <Input
                                            ...
                                            startIcon={
                                                <User className='w-4 h-4 text-[#686764]' />
                                            }
                                        />
                                        <Input
                                            label='Last Name'
                                    ...
                                    <Input
                                        ...
                                        startIcon={
                                            <MapPin className='w-4 h-4 text-[#686764]' />
                                        }
                                    />
                                            ...
                                            <div className='flex items-start gap-3.5'>
                                                ...
                                                <div className='flex flex-col gap-1'>
                                                    ...
                                                    <p className='text-xs text-[#686764] leading-relaxed'>
                                                        Pay securely via Shopify
                                                        Payment Gateway (Google
                                                        Pay, PhonePe, Paytm,
                                                        Credit/Debit Cards,
                                                        NetBanking).
                                                    </p>
                                                ...
                                                <div className='flex flex-col gap-1'>
                                                    <div className='flex items-center gap-2 flex-wrap'>
                                                        ...
                                                        <span className='inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 bg-[#f0edea] text-[#8b7355] border border-[#e8e4df] rounded-full uppercase tracking-wider'>
                                                            <Banknote className='w-3 h-3 text-[#c4622d]' />{" "}
                                                            Pay on Doorstep
                                                        </span>
                                                    </div>
                        ...
                        <div className='bg-[#faf8f5] border border-[#e8e4df] rounded-[8px] p-6 md:p-8 shadow-sm flex flex-col gap-6'>
                            <div className='flex items-center justify-between border-b border-[#e8e4df] pb-4'>
                                <div className='flex items-center gap-2'>
                                    ...
                                    <h2 className='text-base font-bold text-[#060505]'>
                                        Order Summary ({lines.length})
                                    </h2>
                                </div>
                                <Link
                                    to='/cart'
                                    className='text-xs text-[#c4622d] hover:underline font-semibold'>
                                    Edit Cart
                                </Link>
                            </div>
                            ...
                            {paymentMethod === "PREPAID" ? (
                                <div className='p-3.5 bg-[#c4622d]/10 border border-[#c4622d]/20 rounded-[6px] flex items-start gap-2.5'>
                                    ...
                                    <div className='flex flex-col gap-0.5 text-xs'>
                                        ...
                                        <span className='text-[#686764] text-[11px]'>
                                            Auto-applied at Shopify payment step
                                            for all Prepaid orders (UPI /
                                            Cards).
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className='p-3.5 bg-[#8b7355]/10 border border-[#8b7355]/20 rounded-[6px] flex items-start gap-2.5'>
                                    ...
                                    <div className='flex flex-col gap-0.5 text-xs'>
                                        <span className='font-bold text-[#060505]'>
                                            Cash on Delivery Selected
                                        </span>
                                        <span className='text-[#686764] text-[11px]'>
                                            Your order is placed directly
                                            without upfront payment. Pay exact
                                            cash upon delivery.
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div className='pt-4 border-t border-[#e8e4df] flex flex-col gap-2.5 text-xs text-[#686764]'>
                                ...
                                <div className='flex justify-between'>
                                    ...
                                    <span className='text-[#8b7355] font-semibold uppercase'>
                                        Free
                                    </span>
                                </div>
                                <div className='flex justify-between'>
